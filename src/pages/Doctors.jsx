		...
		<div className="">
			...
			<div className="flex flex-col sm:flex-row items-start gap-5 mt-5">
				<div className="flex flex-col gap-4 text-sm text-gray-600">
					...
					<p
						onClick={() =>
							speciality === "Neurologists" ? navigate("/doctors") : navigate("/doctors/Neurologists")
						}
						className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer ${speciality === "Neurologists" ? "bg-indigo-100 text-black" : ""}`}>
						Neurologists
					</p>
					<p
						onClick={() =>
							speciality === "Gastroenterologists" ? navigate("/doctors") : navigate("/doctors/Gastroenterologists")
						}
						className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer ${speciality === "Gastroenterologists" ? "bg-indigo-100 text-black" : ""}`}>
						Gastroenterologists
					</p>
				</div>
					...
					{filterDoc.map((item, index) => {
						return (
							<div
								...
								className="border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] hover:shadow-lg transition-all duration-500"
								key={index}>
